...
def joint_dual_potential_plot(env_tgt, env_src, π_tgt, π_src, φ, ψ, num_actions=4,
    ...
    save_grid_diagnostic_image(env_tgt, π_tgt, horizon_eval, ntraj, "Tπ trajs.", ax=axes[0],show=False)
    save_grid_diagnostic_image(env_src, π_src, horizon_eval, ntraj, "π_src opt trajs.", ax=axes[5],show=False)
    plot_reward_heatmap(env_tgt, φ._f, list(range(num_actions)), 'φ(s,', axes = axes[1:5],show=False)
    plot_reward_heatmap(env_src, ψ._f, list(range(num_actions)), 'ψ(s,', axes = axes[6:],show=False)
    if save_path:
...
def plot_reward_heatmap(env, reward_function, action_index, title='Reward', axes = None, save_path=None, show=True):
    states = []
    ...
    if action_index != 'all':
        rewards = reward_function(states)[:,action_index].detach()
    else:
        rewards = reward_function(states).detach()
...
def save_grid_diagnostic_image(env, policy, num_steps, num_paths, title,
