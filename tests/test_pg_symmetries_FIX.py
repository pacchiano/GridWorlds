...
## Test Tabular Policy
@ray.remote
def run_reversal_experiments(length, height, num_env_steps, success_num_trials, num_pg_steps, stepsize, trajectory_batch_size, manhattan_reward, tabular, location_based, location_normalized, encode_goal, sparsity, 
	...
	base_rewards_randomcore, base_success_num_randomcore = test_policy(env, randomcore_policy, success_num_trials, num_env_steps)
	if not use_ray:
	...
	pg_rewards_randomcore, pg_success_num_randomcore = test_policy(env, randomcore_policy, success_num_trials, num_env_steps)
	if not use_ray:
	...
	base_rewards_symmetries, base_success_num_symmetries = test_policy(env, policy, success_num_trials, num_env_steps)
	...
	pg_rewards_symmetries, pg_success_num_symmetries = test_policy(env, policy, success_num_trials, num_env_steps)
	...
	base_rewards_symmetries_grads, base_success_num_symmetries_grads = test_policy(env, policy, success_num_trials, num_env_steps)
	...
	pg_rewards_symmetries_grads, pg_success_num_symmetries_grads = test_policy(env, policy, success_num_trials, num_env_steps)
...
base_dir = "{}/figs/symmetries/T{}/grid{}_{}/Manhattan{}/".format(path,num_pg_steps, length, height,manhattan_reward)
...
for hidden_layer in [2, 3, 5, 10, 20, 30]:
	...
	if use_ray:
		...
		results = [run_reversal_experiments.remote(length, height, num_env_steps, success_num_trials, num_pg_steps, stepsize, trajectory_batch_size, manhattan_reward, tabular, location_based, location_normalized, encode_goal, sparsity, 
	hidden_layer, "./models/policy_netowrk_symmetries_hidden_{}_expnum_{}.pt".format(hidden_layer, i), base_dir) for i in range(num_experiments)]

		results = ray.get(results)
	else:
		results = [run_reversal_experiments(length, height, num_env_steps, success_num_trials, num_pg_steps, stepsize, trajectory_batch_size, manhattan_reward, tabular, location_based, location_normalized, encode_goal, sparsity, 
	hidden_layer, "./models/policy_netowrk_symmetries_hidden_{}_expnum_{}.pt".format(hidden_layer, i), base_dir) for i in range(num_experiments)]
